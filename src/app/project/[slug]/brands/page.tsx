...
export default function BrandsPage() {
  ...
  useEffect(() => {
    async function load() {
      const res = await fetch(`/api/projects/${slug}/entities?type=brand`);
      if (res.ok) {
  ...
  return (
    <div className="space-y-6 animate-fade-in-up">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">{entities.length} brand menzionati negli articoli</p>
      </div>
      ...
      {entities.length > 0 ? (
              ...
              <CardContent className="p-5">
                ...
                <div className="grid grid-cols-2 gap-2 mb-3">
                  ...
                  <div className="p-2 rounded-lg bg-muted/50 text-center">
                    ...
                    <p className="text-[10px] text-muted-foreground font-medium">Domini</p>
                  </div>
                </div>
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-1">
                      {entity.domains.slice(0, 3).map((d) => (
                        <span key={d} className="text-[11px] bg-accent/10 text-accent px-2 py-0.5 rounded-full">{d}</span>
                      ))}
                ...
                <button
                  ...
                  className="mt-3 w-full text-xs text-accent hover:text-accent/80 font-medium py-1.5 rounded-lg hover:bg-accent/5 transition-colors"
                >
        ...
        <Card className="border-0 shadow-md">
          <CardContent className="p-16 text-center">
            ...
            <h3 className="text-lg font-semibold text-primary mb-1">Nessun brand trovato</h3>
            <p className="text-sm text-muted-foreground">I brand verranno estratti automaticamente dall&apos;analisi AI.</p>
          </CardContent>
        </Card>
      )}
    </div>
